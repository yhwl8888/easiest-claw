...
export function GroupAvatar({ members, size = 40, className }: GroupAvatarProps) {
  ...
  return (
    ...
    >
      {avatars.map((member) => {
        const url = getParticipantAvatarUrl(member.id)
        return url ? (
          <img
            key={member.id}
            src={url}
          ...
          <div
            key={member.id}
            className="w-full h-full flex items-center justify-center rounded-sm bg-blue-100 text-blue-700 font-medium"
            style={{ aspectRatio: "1 / 1", fontSize: size / (cols * 2.5) }}
          >
