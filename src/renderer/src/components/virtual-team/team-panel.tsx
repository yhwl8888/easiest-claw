...
import { Loader2, MessageSquare, Pause, Settings, Trash2 } from "lucide-react"
...
import { AgentFilesSheet } from "./agent-files-sheet"
...
export function TeamPanel({ open, onOpenChange, onStartChat }: TeamPanelProps) {
  ...
  const [filesAgent, setFilesAgent] = useState<Agent | null>(null)
  ...
  return (
    ...
    <AgentFilesSheet
      agent={filesAgent}
      open={filesAgent !== null}
      onOpenChange={(v) => { if (!v) setFilesAgent(null) }}
    />
    </>
  )
}

function AgentCard({ agent, onChat, onFiles }: { agent: Agent; onChat: () => void; onFiles: () => void }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error((res as { error?: string }).error ?? "删除失败，请重试")
        return
      }
    ...
    <>
      <Card className="p-3 gap-2">
        ...
        <div className="flex gap-1">
          ...
          <Button size="sm" variant="outline" className="h-7 text-xs flex-1" onClick={onFiles}>
